...
```{r}
download.file("https://ethz-ins.org/content/Myc_dm_subset.zip", "week04/Myc_dm_subset.zip")
unzip("week04/Myc_dm_subset.zip")
```
...
```{r}
peaks <- rtracklayer::import("week04/Myc_dm_subset/peaks/Myc_peaks.narrowPeak")
plotSignalTracks(list(signal="week04/Myc_dm_subset/aligned/Myc.bam", input="week04/Myc_dm_subset/aligned/input.bam", peaks=peaks),
...
```{r}
dir.create("week04/tracks")
epiwraps::bam2bw("week04/Myc_dm_subset/aligned/Myc.bam", "week04/tracks/Myc_full.bw", binWidth=1, extend=100, scaling=TRUE)
epiwraps::bam2bw("week04/Myc_dm_subset/aligned/Myc.bam", "week04/tracks/Myc.bw", binWidth=20, extend=100, scaling=TRUE)
epiwraps::bam2bw("week04/Myc_dm_subset/aligned/input.bam", "week04/tracks/input.bw", binWidth=20, extend=100, scaling=TRUE)
```

# Looking at an example peak

```{r}
plotSignalTracks(list(full="week04/tracks/Myc_full.bw", binned="week04/tracks/Myc.bw", peaks=peaks), 
...
```{r}
tracks <- list(Myc="week04/tracks/Myc.bw", binned="week04/tracks/input.bw", peaks=peaks)
...
```{r}
peakSignals <- signal2Matrix(c(Myc="week04/tracks/Myc.bw", input="week04/tracks/input.bw"),
...
```{r}
epiwraps::bam2bw("week04/Myc_dm_subset/aligned/Myc.bam", bgbam="week04/Myc_dm_subset/aligned/input.bam", 
                 output_bw="week04/tracks/Myc_relative.bw", binWidth=20, extend=100, scaling=TRUE)
```
...
If we want to extract the subset of peaks 
```{r}
