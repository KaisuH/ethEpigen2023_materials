...
```{r}
dir.create("week03/rfastp.trimmed")
qc <- lapply(c(Myc="week03/raw/Myc.fastq.gz", input="raw/input.fastq.gz"), FUN=function(x){
  Rfastp::rfastp(x, thread=4, overrepresentationAnalysis=TRUE,
                 outputFastq=file.path("week03/rfastp.trimmed/",gsub("\\.fastq\\.gz$","",basename(x))))
})
```
